...
public class RubyMushroomFeature extends Feature<NoneFeatureConfiguration> {

    public RubyMushroomFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            // clé : pousse seulement s'il y a quelque chose au-dessus
            if (level.isEmptyBlock(pos.above())) continue;

            level.setBlock(pos, ModBlocks.RUBY_MUSHROOM.get().defaultBlockState(), 2);
            placedAny = true;
        }
