package com.magius.world.mod.entity.dragonmaid;

import com.magius.world.mod.entity.clan.ClanNpcEntity;
import net.minecraft.world.entity.EntityType;
import net.minecraft.world.entity.PathfinderMob;
import net.minecraft.world.level.Level;

public abstract class DragonmaidEntity extends ClanNpcEntity {

    protected DragonmaidEntity(
            EntityType<? extends PathfinderMob> entityType,
            Level level
    ) {
        super(
                entityType,
                level
        );
    }

    protected abstract String getDragonmaidName();

    @Override
    protected String getClanNpcName() {
        return getDragonmaidName();
    }
}